...
- **v0.6 Hardening** is next: TLS/CA everywhere the v0.5 posture deferred it (agent↔query hops, the custom-metrics APIService CA bundle + front-proxy verification, the #90 validating webhook), HTTP/2 + gRPC capture modules, TLS uprobes, path templating, sampling, cardinality enforcement.
