...
- **v0.4.5 Verification & Soundness** planned next (per [ADR-0023](docs/design/decisions.md#adr-0023-verification-first--v045-milestone-gates-v05), issues [#150](https://github.com/gke-labs/in-cluster-observability/issues/150)–[#158](https://github.com/gke-labs/in-cluster-observability/issues/158)), sequenced after the v0.4 security remediation (#143–#145) lands: Kind e2e presubmit for the DaemonSet, contract fixtures recorded from a real OBI, OBI v0.9.0 → v0.10.0 bump, OTLP temporality/histogram correctness in the metrics path, `pkg/capture` lifecycle fixes, DaemonSet production trim, CI pinning, tracker/PR sweep. v0.5 then proceeds as a vertical slice (tsdb HEAD → PromQL fan-out → custom-metrics API → HPA demo), gated on the library-first posture ADR ([#157](https://github.com/gke-labs/in-cluster-observability/issues/157)).
