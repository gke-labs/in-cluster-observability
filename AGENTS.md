...
- **v0.5 Sinks + Query + HPA** is next, as a vertical slice (tsdb HEAD → PromQL fan-out → custom-metrics API → HPA demo), gated on the library-first posture ADR ([#157](https://github.com/gke-labs/in-cluster-observability/issues/157)).
