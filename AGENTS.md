...
- `cmd/ollie-query/` — v0.5 query server (#94/#95). Stateless Deployment; discovers agents via the `ollie-agent` headless Service, fans PromQL reads out to each agent's remote-read endpoint (`:9091`, stock `storage/remote` client), evaluates centrally with the stock PromQL engine, serves `/api/v1/query` + `/api/v1/query_range` (+ `/healthz/*`) on `:9095` with scrapeauth-style bearer-token auth (`ollie-promql-reader`). Degraded fan-out surfaces as `degraded=true` + `missingNodes` + a warning in the response envelope.
...
- `internal/` — private packages: `obiconfig` (typed OBI YAML schema + atomic writer), `otlpreceiver` (loopback gRPC + HTTP OTLP receivers), `debugendpoint` (loopback PID-control HTTP), `scrapeauth` (in-process TokenReview + SubjectAccessReview middleware, path-keyed — guards agent `/metrics`, agent `/api/v1/read`, and the query server's `/api/v1/*`), `store` (embedded tsdb + registry self-scrape ingester, ADR-0025), `fanout` (query-server cross-agent Queryable + DNS discovery + degraded stats), `queryapi` (Prometheus-compatible HTTP API), `archtest` (enforces OBI import boundary)
