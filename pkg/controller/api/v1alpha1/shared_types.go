...
type HTTPConfig struct {
	...
	// uprobes. Empty adds no ports beyond the agent's default
	// --obi-instrument-ports seed (the controller does not default a
	// per-CR port list). Each port must be in [1, 65535] — enforced by
...
type GRPCConfig struct {
	...
	// adds no ports beyond the agent's default --obi-instrument-ports
	// seed (the controller does not default a per-CR port list). Each
	// port must be in [1, 65535] — enforced by the CRD schema so an out-of-range
